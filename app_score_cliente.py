...
pontuacao_pre += (
    30 if localizacao == "Até 400km"
    else 17.1 if localizacao == "De 400km até 800km"
    else 21.4 if localizacao == "De 900km até 1200km"
    else 12.9
)
...
pontuacao_pre += (
    15 if tamanho == "Acima de 1800m²"
    else 12 if tamanho == "De 1200m² até 1800m²"
    else 9 if tamanho == "De 800m² até 1200m²"
    else 3
)

pontuacao_pre += (
    5 if obra in ["120 a 80 dias", "Acima de 180 dias", "180 a 120 dias"]
    else 0
)

pontuacao_pre += (
    30 if projeto == "Projeto Gera Arte"
    else 24 if projeto == "Caderno/Arq.Parceiro"
    else 12 if projeto == "Caderno"
    else 0
)
...
pontuacao_qual += (
    20 if autoridade == "Decisão com sócio/dono com bom relacionamento"
    else 16 if autoridade == "Decisão com sócio/dono sem relacionamento"
    else 12 if autoridade == "Decisor indireto com acesso fácil"
    else 4
)
...
dados = {
    ...
    "Momento da obra": obra,
    "Já tem projeto": projeto,
    "Pontuação Pré-Qualificação": pontuacao_pre,
    "Categoria Pré-Qualificação": categoria_pre,
    "Aprovação Diretoria": "Sim" if categoria_pre == "C" else "Não se aplica",
    "Concorrência": concorrencia,
    "Desejo e Expectativa": expectativa,
    "Investimento Previsto": investimento,
    "Autoridade de Decisão": autoridade,
    "Quantidade de lojas": quantidade,
    "Materiais Complementares": materiais,
    "Pontuação Qualificação": pontuacao_qual,
    "Categoria Qualificação": categoria_qual,
...
st.download_button(
    ...
    data=csv,
    file_name="resultado_score_cliente.csv",
    mime='text/csv'
)

st.download_button(
    label="🖼️ Baixar Relatório em PNG",
    data=byte_im,
    file_name="relatorio_score_cliente.png",
    mime="image/png"
)